...
\begin{definition}[Quotient Group]\index{quotient group}\label{def:quotient-group}
    Let $G$ be a group and $N$ a normal subgroup. Then, we can define the \term{quotient group} $G/N$ as the set of left \hyperref[def:coset]{cosets} of $N$ in $G$ with the operation \[
        (gN)(hN) := (gh)N.
    \]
\end{definition}
...
\includepdf[pages=-,nup=3x5]{pdfs/MAT301 S 2024 Lecture 10.pdf}